...
export const getGreenhouseByOwnerAndAlias = async (req, res) => {
	const verifiedToken = verifyUserToken(req);
	if (!verifiedToken.success) {
		return res.status(verifiedToken.status).json(verifiedToken);
	}
	...
	try {
		const result = await greenhouseService.getGreenhouseByOwnerIdAndAlias(user.id, alias);

		if (!result) {
			return res.status(404).json({
				status: 404,
				error: 'NotFound',
				message: `Greenhouse with alias '${alias}' not found for this user`,
				data: null
			});
		}
		...
		res.status(200).json({
			status: 200,
			error: '',
			message: 'Greenhouse fetched successfully',
			data: result
		});
	} catch (err) {
		Log.bad(`Error fetching greenhouse: ${err.message}`);
		Log.bad(err.stack);
		res.status(500).json({ status: 500, error: 'Internal Server Error', message: err.message });
	}
};
